...
- 悬浮交通灯：超窄竖排设计，默认 80 × 260 点；拖动灯体可移动，拖动右下角抓手可等比例缩放并记住尺寸。
- 呼吸光晕：当前亮起的灯会轻微呼吸闪动，持续表达运行状态，不干扰红灯报警闪烁规则。
- Codex Hooks 集成：根据 Codex 的 hook 事件自动更新任务状态。
- 命令行控制：可手动设置状态、查询状态、清空状态或退出应用。
- 多任务聚合：多个 Codex 任务同时存在时，按优先级显示最需要关注的状态。
- 周额度展示：浮窗底部可显示 Codex 周剩余额度百分比。
...
启动后，macOS 菜单栏会出现 Codex 状态灯。红灯或绿灯时会自动显示悬浮交通灯；黄灯默认只在菜单栏显示，减少打扰。悬浮灯可直接拖动位置，右下角斜线抓手用于等比例缩放，调整后的尺寸会在下次启动时恢复。

开发截图或视觉验收时，应通过菜单中的“显示/隐藏红绿灯”展示真实状态；不要向正式状态文件写入临时 `waiting` 任务，以免测试红灯被误认为真实待确认。
...
codex-light-mxp quota --weekly 48
printf '%s' '{"quota":{"weekly_remaining_percent":48}}' | codex-light-mxp quota --stdin
```
...
codex-light-mxp quota --weekly 48 --json
...
- `quota --app-server`：通过本机 Codex app-server 的 `account/rateLimits/read` 读取周额度，并写入状态文件。
- `quota --weekly <0-100>`：手动更新浮窗底部显示的 Codex 周剩余额度百分比。
- `quota --stdin`：从标准输入读取 JSON，提取 `weekly_remaining_percent` 或 `weeklyRemainingPercent`。

App 启动后会自动尝试采集一次额度，之后每 60 秒轮询一次。采集依赖 Codex CLI 的实验性 `app-server` 协议；如果 Codex 升级导致协议变化或网络波动，采集失败时会保留上一次额度，不会清空现有显示。
如果周剩余额度为 `0%`，且 app-server 返回了对应 `resetsAt` 时间，菜单栏会显示“还有多久”恢复，而不是只显示绝对时间。
...
如果浮窗显示 `--`，通常说明当前状态文件还没有 `quota` 字段，或者本机 Codex app-server 暂时没有返回可识别的周额度。可以先手动触发采集：
...
本工具不会读取 Codex 私有数据库、私有日志或 auth token。app-server 不可用时，可以临时用 `codex-light-mxp quota --weekly 48` 手动写入。
...
Codex Desktop / app-server 会话有时不经过 CLI hook dispatcher。可以安装一个本机 LaunchAgent，轮询 Codex 本地日志数据库：检测到正在运行的桌面 turn 时写入黄灯，日志停止一小段时间后写入绿灯。Hooks 正常送达时，该监控会自动让位，不会用黄灯覆盖 Hook 的绿灯或红灯。周额度由菜单栏 App 独立采集，不参与任务灯色判定。
...
CODEX_TRAFFIC_LIGHT_STATE_PATH="$STATE" codex-light-mxp quota --weekly 48
printf '%s' '{"quota":{"weeklyRemainingPercent":47}}' \
