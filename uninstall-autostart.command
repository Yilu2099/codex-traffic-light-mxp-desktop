...
MONITOR_PLIST="$HOME/Library/LaunchAgents/com.codex.traffic-light-codex-monitor.plist"
...
for PLIST in "$APP_PLIST" "$UPDATER_PLIST" "$MONITOR_PLIST"; do
  if [[ -e "$PLIST" ]]; then
    /bin/mkdir -p "$TRASH_DIR"
    /bin/mv "$PLIST" "$TRASH_DIR/"
  fi
done
echo "状态栏、项目审计监控和自动更新已停止。LaunchAgent 文件已移到废纸篓，安装版本仍保留。"
