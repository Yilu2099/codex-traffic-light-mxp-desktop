...
func testClientReleaseRetentionPinsActiveBridgeRollback() throws {
    let tokenRoot = FileManager.default.temporaryDirectory
        .appendingPathComponent("codex-release-bridge-retention-\(UUID().uuidString)", isDirectory: true)
    let root = tokenRoot.appendingPathComponent("app", isDirectory: true)
    let releases = root.appendingPathComponent("releases", isDirectory: true)
    defer { try? FileManager.default.removeItem(at: tokenRoot) }
    for version in ["1.2.84", "1.2.91", "1.2.97"] {
    ...
    try FileManager.default.createSymbolicLink(
        atPath: root.appendingPathComponent("current").path,
        withDestinationPath: "releases/1.2.97"
    )
    try "1.2.97\nreleases/1.2.84\n".write(
        to: root.appendingPathComponent("launch-agent-bridge.request"),
        atomically: true,
        encoding: .utf8
    )

    _ = ClientReleaseRetention.prune(appRoot: root, currentVersion: "1.2.97")
    try expect(FileManager.default.fileExists(atPath: releases.appendingPathComponent("1.2.84").path), "active bridge request must pin its real 1.2.84 rollback release")

    try FileManager.default.removeItem(at: root.appendingPathComponent("launch-agent-bridge.request"))
    _ = ClientReleaseRetention.prune(appRoot: root, currentVersion: "1.2.97")
    try expect(!FileManager.default.fileExists(atPath: releases.appendingPathComponent("1.2.84").path), "old rollback may be pruned only after the bridge request is gone")
}
...
func testDesktopMonitorArchivesStaleBridgeAndPreparesCurrentRelease() throws {
    ...
    let monitor = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        .appendingPathComponent("scripts/codex-light-codex-monitor")
    let process = Process()
    process.executableURL = URL(fileURLWithPath: "/bin/zsh")
    process.arguments = [monitor.path]
    process.environment = [
        "HOME": home.path,
        "PATH": "/usr/bin:/bin:/usr/sbin:/sbin",
        "CODEX_LIGHT_MONITOR_ONCE": "1",
        "CODEX_LIGHT_LOG_DB": root.appendingPathComponent("missing-log.sqlite").path,
        "CODEX_LIGHT_STATE_DB": root.appendingPathComponent("missing-state.sqlite").path,
    ]
    try process.run()
    process.waitUntilExit()
    try expectEqual(process.terminationStatus, 0, "monitor stale-request recovery fixture must finish")
    try expectEqual(
        try String(contentsOf: appRoot.appendingPathComponent("launch-agent-bridge-stale.failed"), encoding: .utf8),
        "1.2.93\nreleases/1.2.84\n",
...
func testMainAppLaunchAgentForceRebootstrapsUnchangedSignedTarget() throws {
    ...
    var commands: [[String]] = []
    var bootoutSeen = false
    let runner: ([String]) -> MainAppLaunchctlResult = { arguments in
        commands.append(arguments)
        if arguments.first == "bootout" { bootoutSeen = true }
        if arguments.first == "print", bootoutSeen {
            return MainAppLaunchctlResult(status: 3, output: "Could not find service")
        }
    ...
    ], "forced activation must bootout and bootstrap even when bootout reports an absent job")
    ...
    try expectEqual(commands.map(\.first), ["print", "bootout", "print", "bootstrap"], "repeated signed activation must prove removal before refreshing registration")
}
...
func testMainAppLaunchAgentRepairsMissingJobAndSurfacesBootstrapFailure() throws {
    ...
    let repaired = try MainAppLaunchAgentInstaller.install(
        ...
        runLaunchctl: { arguments in
            commands.append(arguments)
            if arguments.first == "print" { return MainAppLaunchctlResult(status: 113, output: "Could not find service") }
            return MainAppLaunchctlResult(status: 0)
        }
    )
    try expect(repaired, "a missing registered job should be restored")
    try expectEqual(commands.map(\.first), ["print", "bootout", "print", "bootstrap"], "missing-job repair must prove absence before registration")

    var bootoutCompleted = false
    do {
        _ = try MainAppLaunchAgentInstaller.install(
            ...
            forceRebootstrap: true,
            runLaunchctl: { arguments in
                if arguments.first == "bootout" { bootoutCompleted = true }
                if arguments.first == "print", bootoutCompleted {
                    return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                }
                if arguments.first == "bootstrap" {
                    return MainAppLaunchctlResult(status: 9, output: "simulated non-EIO bootstrap rejection")
                }
                return MainAppLaunchctlResult(status: 0)
            }
        )
...
func testLaunchAgentInstallersRetryEIOOnlyAfterConfirmedAbsence() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-eio-retry-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let release = home.appendingPathComponent(".wanhe-codex-token/app/current")
    try FileManager.default.createDirectory(at: release, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    try "__APP_PATH__\n__HOME__\n".write(
        to: release.appendingPathComponent("\(MainAppLaunchAgentInstaller.label).plist.template"),
        atomically: true,
        encoding: .utf8
    )
    try "__UPDATER_PATH__\n__HOME__\n".write(
        to: release.appendingPathComponent("\(UpdaterLaunchAgentInstaller.label).plist.template"),
        atomically: true,
        encoding: .utf8
    )

    do {
        let service = "gui/516/\(MainAppLaunchAgentInstaller.label)"
        var bootouts = 0
        var bootstraps = 0
        var pauses = 0
        var absenceChecks = 0
        let installed = try MainAppLaunchAgentInstaller.install(
            ...
            userID: 516,
            forceRebootstrap: true,
            bootstrapRetryPause: { pauses += 1 },
            runLaunchctl: { arguments in
                if arguments == ["print", service] {
                    if bootouts == 0 { return MainAppLaunchctlResult(status: 0, output: "state = running") }
                    absenceChecks += 1
                    if bootstraps == 0, absenceChecks <= 2 {
                        return MainAppLaunchctlResult(status: 0, output: "state = waiting")
                    }
                    return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                }
                if arguments == ["bootout", service] {
                    bootouts += 1
                    return MainAppLaunchctlResult(status: 0)
                }
                if arguments.first == "bootstrap" {
                    bootstraps += 1
                    return bootstraps == 1
                        ? MainAppLaunchctlResult(status: 5, output: "Bootstrap failed: 5: Input/output error")
                        : MainAppLaunchctlResult(status: 0)
                }
                return MainAppLaunchctlResult(status: 0)
            }
        )
        try expect(installed, "main installer should recover from one transient EIO")
        try expectEqual(bootstraps, 2, "main bootstrap should retry exactly once after EIO")
        try expectEqual(bootouts, 2, "EIO retry must start with a fresh bootout")
        try expectEqual(absenceChecks, 5, "bootstrap must wait through registered states, then re-prove absence around EIO retry")
        try expectEqual(pauses, 3, "two removal polls and one EIO backoff must remain bounded")
    }

    do {
        let service = "gui/517/\(UpdaterLaunchAgentInstaller.label)"
        var bootouts = 0
        var bootstraps = 0
        var absenceChecks = 0
        let installed = try UpdaterLaunchAgentInstaller.install(
            from: release,
            home: home,
            userID: 517,
            forceRebootstrap: true,
            ...
            runLaunchctl: { arguments in
                if arguments == ["print", service] {
                    if bootouts == 0 { return MainAppLaunchctlResult(status: 0, output: "state = exited") }
                    absenceChecks += 1
                    return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                }
                if arguments == ["bootout", service] {
                    bootouts += 1
                    return MainAppLaunchctlResult(status: 5, output: "Boot-out failed: 5: Input/output error")
                }
                if arguments.first == "bootstrap" {
                    bootstraps += 1
                    return bootstraps == 1
        ...
        try expect(installed, "updater installer may continue after bootout EIO only when launchd proves the job absent")
        try expectEqual(bootstraps, 2, "updater bootstrap should also retry exactly once after EIO")
        try expectEqual(bootouts, 2, "updater EIO retry must perform a fresh bootout")
        try expectEqual(absenceChecks, 4, "bootout and bootstrap EIO paths must each prove absence around backoff")
    }

    do {
        let service = "gui/518/\(MainAppLaunchAgentInstaller.label)"
        var bootouts = 0
        var bootstraps = 0
        do {
            _ = try MainAppLaunchAgentInstaller.install(
                ...
                runLaunchctl: { arguments in
                    ...
                    if arguments.first == "bootstrap" {
                        bootstraps += 1
                        return MainAppLaunchctlResult(status: 5, output: "Bootstrap failed: 5: Input/output error")
                    }
                    return MainAppLaunchctlResult(status: 0)
                }
            )
            throw TestFailure(description: "a still-registered service must prohibit EIO bootstrap retry")
        } catch let error as MainAppLaunchAgentError {
            try expect(error.description.contains("bootout"), "persistent registration must be reported as removal failure")
        }
        try expectEqual(bootouts, 1, "initial accepted bootout must be followed by explicit absence verification")
        try expectEqual(bootstraps, 0, "service still registered must prevent even the first bootstrap")
    }
}
...
func testLaunchAgentBridgeWaitsForOldUpdaterAndRefreshesBothJobs() throws {
    ...
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    try FileManager.default.createSymbolicLink(atPath: appRoot.appendingPathComponent("current").path, withDestinationPath: "releases/\(ClientVersion.current)")
    try "\(ClientVersion.current)\n".write(to: target.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "new updater".write(to: target.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    try "1.2.90\n".write(to: previous.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "old main".write(to: previous.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    ...
    var absentServices: Set<String> = []
    let result = try LaunchAgentUpdateBridge.run(
        targetVersion: "\(ClientVersion.current)",
        ...
        runLaunchctl: { arguments in
            commands.append(arguments)
            if arguments.first == "bootout", arguments.count == 2 {
                absentServices.insert(arguments[1])
                return MainAppLaunchctlResult(status: 0)
            }
            if arguments.first == "print", arguments.count == 2, absentServices.contains(arguments[1]) {
                return MainAppLaunchctlResult(status: 3, output: "Could not find service")
            }
            if arguments.first == "bootstrap" {
                if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                    absentServices.remove("gui/503/\(MainAppLaunchAgentInstaller.label)")
                }
                if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                    absentServices.remove("gui/503/\(UpdaterLaunchAgentInstaller.label)")
                }
            }
        ...
        takeOverLegacyUpdater: { previousTarget in
            try expect(commands.isEmpty, "legacy updater takeover must finish before launchd registration changes")
            takeoverTargets.append(previousTarget)
        }
    )
    ...
    try expectEqual(takeoverTargets, ["releases/1.2.90"], "legacy bridge must take ownership from the exact predecessor")
    ...
    try expect(FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "successful bridge should persist an idempotency marker")
...
func testLaunchAgentBridgeRecoversBeforeSwapAndAfterLegacyRollback() throws {
    let scenarios: [(predecessorMovesFirst: Bool, mainBootstrapFails: Bool)] = [
        (false, false), (true, false), (false, true),
    ]
    for scenario in scenarios {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-recover-\(scenario.predecessorMovesFirst)-\(scenario.mainBootstrapFails)-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
        let previous = appRoot.appendingPathComponent("releases/1.2.84")
        let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
        let launchAgents = home.appendingPathComponent("Library/LaunchAgents")
        try FileManager.default.createDirectory(at: previous, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: target, withIntermediateDirectories: true)
        ...
        try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/1.2.84")
        try "1.2.84".write(to: previous.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
        try "old main".write(to: previous.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
        try "old updater".write(to: previous.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
        try "\(ClientVersion.current)".write(to: target.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
        try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
        try "new updater".write(to: target.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
        try "__APP_PATH__\n__HOME__\n".write(to: target.appendingPathComponent("com.codex.traffic-light-mxp.plist.template"), atomically: true, encoding: .utf8)
        try "__UPDATER_PATH__\n__HOME__\n".write(to: target.appendingPathComponent("com.codex.traffic-light-mxp-updater.plist.template"), atomically: true, encoding: .utf8)
        ...
        var updaterBootstrapped = false
        var mainBootstrapAttempts = 0
        var absentServices: Set<String> = []
        do {
            let result = try LaunchAgentUpdateBridge.run(
                targetVersion: "\(ClientVersion.current)",
                previousTarget: "releases/1.2.84",
                home: home,
                userID: 511,
                ...
                runLaunchctl: { arguments in
                    ...
                    if arguments.first == "bootstrap" {
                        if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                            absentServices.remove("gui/511/\(MainAppLaunchAgentInstaller.label)")
                        }
                        if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                            absentServices.remove("gui/511/\(UpdaterLaunchAgentInstaller.label)")
                        }
                    }
                    if arguments == ["print", "gui/511/com.codex.traffic-light-mxp"] {
                        return MainAppLaunchctlResult(status: 0, output: "state = running")
                    }
                    if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp.plist") == true {
                        mainBootstrapAttempts += 1
                        if scenario.mainBootstrapFails && mainBootstrapAttempts == 1 {
                            return MainAppLaunchctlResult(status: 9, output: "simulated non-EIO target rejection")
                        }
                    }
                    ...
                    if arguments == ["print", "gui/511/com.codex.traffic-light-mxp-updater"] {
                        return updaterBootstrapped
                            ? MainAppLaunchctlResult(status: 0, output: "state = exited\nlast exit code = 0")
                            : MainAppLaunchctlResult(status: 3, output: "Could not find service")
                    }
                    return MainAppLaunchctlResult(status: 0)
                },
                takeOverLegacyUpdater: { _ in
                    if scenario.predecessorMovesFirst {
                        try FileManager.default.removeItem(at: current)
                        try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
                    }
                }
            )
            try expect(!scenario.mainBootstrapFails, "failed recovery activation must not report success")
            try expectEqual(result, .completed, "target helper must recover whether it sees the pre-swap release or the predecessor's concurrent swap")
            try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "legacy recovery must finish on the signed target")
            try expect(FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "legacy recovery must write done only after both jobs pass")
        } catch let error as LaunchAgentUpdateBridgeError {
            try expect(scenario.mainBootstrapFails, "only the injected activation failure should enter rollback: \(error)")
            try expect(error.description.contains("activation failed"), "recovery activation failure must remain diagnosable")
            try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.84", "failed recovery must restore the verified predecessor")
            try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "failed recovery must not write done")
            }
    }
}
...
func testLaunchAgentBridgeRollsBackAndRejectsLateUnsafeCapture() throws {
    ...
    try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "new updater".write(to: target.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    try "1.2.90".write(to: previous.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "old main".write(to: previous.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "old updater".write(to: previous.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            runLaunchctl: { arguments in
                ...
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp.plist") == true {
                    absentServices.remove("gui/504/\(MainAppLaunchAgentInstaller.label)")
                    mainBootstrapCount += 1
                    if mainBootstrapCount == 1 { return MainAppLaunchctlResult(status: 9, output: "simulated non-EIO rejection") }
                }
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                    absentServices.remove("gui/504/\(UpdaterLaunchAgentInstaller.label)")
                }
                return MainAppLaunchctlResult(status: 0)
            },
            takeOverLegacyUpdater: { _ in }
    ...
    let failedAttempt = UpdateLedger(url: home.appendingPathComponent(".wanhe-codex-token/update-attempts.json")).load()
    try expectEqual(failedAttempt?.version, "\(ClientVersion.current)", "bridge must persist target backoff before the rollback updater starts")

    try FileManager.default.removeItem(at: current)
    try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
    var targetMainEIOBootstraps = 0
    var targetMainBootouts = 0
    var successfulRollbackBootstraps = 0
    var rollbackAbsentServices: Set<String> = []
    do {
        _ = try LaunchAgentUpdateBridge.run(
            targetVersion: "\(ClientVersion.current)",
            previousTarget: "releases/1.2.90",
            ...
            userID: 504,
            runLaunchctl: { arguments in
                let currentTarget = try? FileManager.default.destinationOfSymbolicLink(atPath: current.path)
                let mainService = "gui/504/\(MainAppLaunchAgentInstaller.label)"
                if currentTarget == "releases/1.2.90", arguments.first == "bootout", arguments.count == 2 {
                    rollbackAbsentServices.insert(arguments[1])
                    return MainAppLaunchctlResult(status: 0)
                }
                if currentTarget == "releases/1.2.90", arguments.first == "print", arguments.count == 2,
                   rollbackAbsentServices.contains(arguments[1]) {
                    return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                }
                if arguments == ["print", mainService] {
                    if currentTarget == "releases/\(ClientVersion.current)", targetMainBootouts >= 1 {
                        return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                    }
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                if arguments == ["bootout", mainService], currentTarget == "releases/\(ClientVersion.current)" {
                    targetMainBootouts += 1
                    return MainAppLaunchctlResult(status: 0)
                }
                if arguments.first == "bootstrap" {
                    if currentTarget == "releases/\(ClientVersion.current)",
                       arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                        targetMainEIOBootstraps += 1
                        return MainAppLaunchctlResult(status: 5, output: "Bootstrap failed: 5: Input/output error")
                    }
                    if currentTarget == "releases/1.2.90" {
                        if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                            rollbackAbsentServices.remove("gui/504/\(MainAppLaunchAgentInstaller.label)")
                        }
                        if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                            rollbackAbsentServices.remove("gui/504/\(UpdaterLaunchAgentInstaller.label)")
                        }
                        successfulRollbackBootstraps += 1
                    }
                }
                return MainAppLaunchctlResult(status: 0)
            },
        ...
        throw TestFailure(description: "persistent target EIO must roll back")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("activation failed"), "persistent target EIO must preserve the activation failure")
    }
    try expectEqual(targetMainEIOBootstraps, 2, "target main should receive only one controlled EIO retry")
    try expectEqual(successfulRollbackBootstraps, 2, "persistent target EIO must restore both previous launch agents")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.90", "persistent target EIO must restore previous current")

    try FileManager.default.removeItem(at: current)
    try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
    var updaterBootstrapCount = 0
    var rollbackMainBootstraps = 0
    absentServices.removeAll()
    do {
        _ = try LaunchAgentUpdateBridge.run(
            targetVersion: "\(ClientVersion.current)",
            previousTarget: "releases/1.2.90",
            ...
            runLaunchctl: { arguments in
                ...
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp-updater.plist") == true {
                    ...
                    if updaterBootstrapCount == 1 { return MainAppLaunchctlResult(status: 9, output: "simulated non-EIO updater rejection") }
                }
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp.plist") == true {
                    absentServices.remove("gui/504/\(MainAppLaunchAgentInstaller.label)")
    ...
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "failed bridge must never write a completion marker")

    try FileManager.default.removeItem(at: current)
    try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
    var rollbackUpdaterEIOs = 0
    var rollbackMainWasAttempted = false
    absentServices.removeAll()
    do {
        _ = try LaunchAgentUpdateBridge.run(
            targetVersion: "\(ClientVersion.current)",
            ...
            runLaunchctl: { arguments in
                ...
                if arguments == ["print", "gui/504/\(MainAppLaunchAgentInstaller.label)"] {
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                if arguments == ["print", "gui/504/\(UpdaterLaunchAgentInstaller.label)"] {
                    return currentTarget == "releases/1.2.90"
                        ? MainAppLaunchctlResult(status: 3, output: "Could not find service")
                        : MainAppLaunchctlResult(status: 0, output: "state = exited")
                }
                if arguments.first == "bootstrap",
                   arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true,
                   currentTarget == "releases/1.2.90" {
                    absentServices.remove("gui/504/\(UpdaterLaunchAgentInstaller.label)")
                    rollbackUpdaterEIOs += 1
                    return MainAppLaunchctlResult(status: 5, output: "Bootstrap failed: 5: Input/output error")
                }
                if arguments.first == "bootstrap",
                   arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                    if currentTarget == "releases/1.2.90" {
                        absentServices.remove("gui/504/\(MainAppLaunchAgentInstaller.label)")
                        rollbackMainWasAttempted = true
                        return MainAppLaunchctlResult(status: 0)
                    }
                    return MainAppLaunchctlResult(status: 9, output: "simulated target main failure")
                }
                return MainAppLaunchctlResult(status: 0)
            },
            takeOverLegacyUpdater: { _ in }
        )
        throw TestFailure(description: "persistent rollback updater EIO must remain a rollback failure")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("rollback failed"), "persistent rollback EIO must remain visible")
    }
    try expectEqual(rollbackUpdaterEIOs, 2, "rollback updater should receive only one controlled EIO retry")
    try expect(rollbackMainWasAttempted, "rollback main restoration must run even when updater retry is exhausted")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.90", "rollback EIO must retain the verified previous current target")

    var touchedLaunchd = false
    do {
        _ = try LaunchAgentUpdateBridge.run(
            targetVersion: "\(ClientVersion.current)",
            previousTarget: "releases/\(ClientVersion.current)",
            home: home,
            userID: 504,
...
func testLaunchAgentBridgeRefusesMissingRollbackRelease() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-missing-rollback-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
    let previous = appRoot.appendingPathComponent("releases/1.2.84")
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            userID: 505,
            runLaunchctl: { arguments in
                if arguments == ["bootout", "gui/505/\(MainAppLaunchAgentInstaller.label)"] {
                    mainWasBootedOut = true
                    return MainAppLaunchctlResult(status: 0)
                }
                if arguments == ["print", "gui/505/\(MainAppLaunchAgentInstaller.label)"], mainWasBootedOut {
                    return MainAppLaunchctlResult(status: 3, output: "Could not find service")
                }
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp.plist") == true {
                    mainBootstrapCount += 1
                    if mainBootstrapCount == 1 {
                        // Model the real race where startup retention removed
                        // the legacy updater's rollback release.
        ...
        throw TestFailure(description: "a vanished rollback release must not produce a dead current link")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("rollback failed"), "missing rollback release must be surfaced as a terminal rollback failure")
    }
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "bridge must leave current on the existing target instead of swapping to a missing release")
    try expectEqual(mainBootstrapCount, 1, "bridge must revalidate rollback contents before any rollback launchd mutation")
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "failed rollback must never write a completion marker")
}

func testLaunchAgentBridgeRejectsNonOlderPreviousBeforeLaunchd() throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("launch-agent-bridge-version-order-\(UUID().uuidString)")
    defer { try? FileManager.default.removeItem(at: root) }
    for previous in ["releases/\(ClientVersion.current)", "releases/\(ClientVersion.current).1", "releases/1.2.bad", "releases/1.2.999999999999999999999"] {
        var touchedLaunchd = false
        do {
            _ = try LaunchAgentUpdateBridge.run(
                targetVersion: "\(ClientVersion.current)",
            ...
            throw TestFailure(description: "non-older previous version should be rejected: \(previous)")
        } catch LaunchAgentUpdateBridgeError.unsafePreviousTarget {
            try expect(!touchedLaunchd, "invalid version ordering must be rejected before launchd: \(previous)")
        }
    }
}

func testLaunchAgentBridgePreemptsSelfBridgingUpdater() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-self-bridging-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
    let previous = appRoot.appendingPathComponent("releases/1.2.93")
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    let launchAgents = home.appendingPathComponent("Library/LaunchAgents")
    try FileManager.default.createDirectory(at: previous, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: target, withIntermediateDirectories: true)
    ...
    defer { try? FileManager.default.removeItem(at: root) }
    let current = appRoot.appendingPathComponent("current")
    try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/1.2.93")
    try "1.2.93".write(to: previous.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "old main".write(to: previous.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "old updater".write(to: previous.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    try "\(ClientVersion.current)".write(to: target.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    ...
    var newUpdaterWasBootstrapped = false
    var newUpdaterPolls = 0
    var commands: [[String]] = []
    var absentServices: Set<String> = []
    let result = try LaunchAgentUpdateBridge.run(
        targetVersion: "\(ClientVersion.current)",
        previousTarget: "releases/1.2.93",
        home: home,
        userID: 506,
        maximumUpdaterPolls: 0,
        allowActivationFromPrevious: true,
        ...
        runLaunchctl: { arguments in
            ...
            if arguments.first == "bootstrap" {
                if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                    absentServices.remove("gui/506/\(MainAppLaunchAgentInstaller.label)")
                }
                if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                    absentServices.remove("gui/506/\(UpdaterLaunchAgentInstaller.label)")
                }
            }
            if arguments == ["print", "gui/506/com.codex.traffic-light-mxp"] {
                return MainAppLaunchctlResult(status: 0, output: "state = running")
            }
            if arguments == ["print", "gui/506/com.codex.traffic-light-mxp-updater"] {
                if !newUpdaterWasBootstrapped {
                    return MainAppLaunchctlResult(status: 0, output: "state = exited\nlast exit code = 0")
                }
                newUpdaterPolls += 1
                return MainAppLaunchctlResult(
                    status: 0,
                    output: newUpdaterPolls <= 20 ? "state = running" : "state = exited\nlast exit code = 0"
                )
            }
            if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp-updater.plist") == true {
                newUpdaterWasBootstrapped = true
            }
            return MainAppLaunchctlResult(status: 0)
        },
        takeOverLegacyUpdater: { previousTarget in
            try expectEqual(previousTarget, "releases/1.2.93", "pre-swap takeover must stay bound to the actual current predecessor")
            // Model the hardest interleaving: the predecessor already changed
            // current and completed one main registration before its whole
            // process group was retired. The target must converge by doing one
            // final force rebootstrap after exclusive takeover.
            try FileManager.default.removeItem(at: current)
            try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
            takeoverCalled = true
        }
    )
    try expectEqual(result, .completed, "the target helper should own the complete pre-swap transaction")
    try expect(takeoverCalled, "1.2.91+ predecessors must be retired before either target LaunchAgent is changed")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "the target transaction must converge after a predecessor swap interleaving")
    let mainService = "gui/506/\(MainAppLaunchAgentInstaller.label)"
    try expect(commands.contains(["bootout", mainService]), "target takeover must remove a possibly completed predecessor main registration")
    try expect(
        commands.contains { $0.first == "bootstrap" && $0.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true },
        "target takeover must converge by bootstrapping main from the verified target"
    )
    try expect(newUpdaterPolls > 13, "a legitimate updater run longer than the old three-second window must be allowed to finish")
}

func testLaunchAgentBridgeEnforcesWallClockDeadline() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-deadline-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
    let previous = appRoot.appendingPathComponent("releases/1.2.91")
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    let launchAgents = home.appendingPathComponent("Library/LaunchAgents")
    try FileManager.default.createDirectory(at: previous, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: target, withIntermediateDirectories: true)
    ...
    try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "new updater".write(to: target.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    try "1.2.91".write(to: previous.appendingPathComponent("VERSION"), atomically: true, encoding: .utf8)
    try "old main".write(to: previous.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
    try "old updater".write(to: previous.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
    try "__APP_PATH__".write(to: target.appendingPathComponent("com.codex.traffic-light-mxp.plist.template"), atomically: true, encoding: .utf8)
    try "__UPDATER_PATH__".write(to: target.appendingPathComponent("com.codex.traffic-light-mxp-updater.plist.template"), atomically: true, encoding: .utf8)
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            userID: 507,
            maximumDuration: 0.1,
            pause: { Thread.sleep(forTimeInterval: 0.55) },
            runLaunchctl: { arguments in
                if arguments.first == "bootout", arguments.count == 2 {
                    absentServices.insert(arguments[1])
                    return MainAppLaunchctlResult(status: 0)
                ...
                if arguments.first == "bootstrap" {
                    if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                        absentServices.remove("gui/507/\(MainAppLaunchAgentInstaller.label)")
                    }
                    if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                        absentServices.remove("gui/507/\(UpdaterLaunchAgentInstaller.label)")
                    }
                }
                if arguments == ["print", "gui/507/com.codex.traffic-light-mxp"] {
                    return MainAppLaunchctlResult(status: 0, output: "state = waiting")
                }
                return MainAppLaunchctlResult(status: 0)
            },
        ...
        throw TestFailure(description: "bridge should not poll beyond its wall-clock deadline")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("hard deadline"), "deadline expiry must remain explicit in the bridge failure")
    }
    try expect(Date().timeIntervalSince(started) < 2, "deadline path must terminate promptly instead of exhausting its poll count")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.91", "deadline failure should use the still-validated rollback release")
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "deadline failure must not write a completion marker")
}

func testLaunchAgentBridgeStopsWhenPredecessorTakeoverFails() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-takeover-failure-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
    let previous = appRoot.appendingPathComponent("releases/1.2.84")
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            takeOverLegacyUpdater: { _ in throw LaunchAgentUpdateBridgeError.legacyUpdaterTakeoverFailed }
        )
        throw TestFailure(description: "failed predecessor ownership must enter the bridge failure transaction")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("activation failed"), "takeover failure should remain diagnosable")
    }
    try expect(commands.isEmpty, "unproven predecessor ownership must block every current and launchd mutation")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "post-swap takeover failure must leave current untouched for formal repair")
    try expectEqual(UpdateLedger(url: home.appendingPathComponent(".wanhe-codex-token/update-attempts.json")).load()?.version, "\(ClientVersion.current)", "takeover failure must persist retry backoff")
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "takeover failure must never write done")
    ...
    do {
        ...
        throw TestFailure(description: "pre-swap takeover failure must require formal repair")
    } catch LaunchAgentUpdateBridgeError.activationFailed(_) {
        try expect(commands.isEmpty, "pre-swap takeover failure must not touch a healthy previous main or updater")
        try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.84", "pre-swap takeover failure must leave current on the working predecessor")
    }
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            runLaunchctl: { arguments in commands.append(arguments); return MainAppLaunchctlResult(status: 0) },
            takeOverLegacyUpdater: { _ in
                throw LaunchAgentUpdateBridgeError.legacyUpdaterProcessGroupStillRunning(700)
            }
        )
        throw TestFailure(description: "a surviving legacy process group must require formal repair")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("process group 700 remained"), "partial takeover must preserve the terminal diagnostic")
    }
    try expect(commands.isEmpty, "partial takeover failure must not mutate current or either launchd job")
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "partial takeover failure must leave current untouched")
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            userID: 508,
            runLaunchctl: { arguments in commands.append(arguments); return MainAppLaunchctlResult(status: 0) },
            takeOverLegacyUpdater: { _ in throw LaunchAgentUpdateBridgeError.legacyUpdaterServiceStillRegistered }
        )
        throw TestFailure(description: "a registered predecessor service must require formal repair")
    } catch LaunchAgentUpdateBridgeError.activationFailed(_) {
        try expect(commands.isEmpty, "unremoved predecessor service must block every rollback mutation")
        try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/\(ClientVersion.current)", "unremoved predecessor service must leave current untouched")
    }
}

func testLegacyUpdaterTakeoverWaitsForWholeProcessGroup() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("legacy-updater-process-group-\(UUID().uuidString)")
    let release = root.appendingPathComponent("releases/1.2.84")
    let executable = release.appendingPathComponent("wanhe-status-updater")
    let plist = root.appendingPathComponent("com.codex.traffic-light-mxp-updater.plist")
    try FileManager.default.createDirectory(at: release, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    try "updater".write(to: executable, atomically: true, encoding: .utf8)
    let validPlist: [String: Any] = [
        "Label": UpdaterLaunchAgentInstaller.label,
        "ProgramArguments": [executable.path],
    ]
    try PropertyListSerialization.data(fromPropertyList: validPlist, format: .xml, options: 0).write(to: plist)
    let snapshot = RunningUpdaterProcess(pid: 700, executable: executable)
    var groupChecks = 0
    var checkedGroups: [pid_t] = []
    var pauses = 0
    ...
    try LegacyUpdaterProcessGroupTakeover.run(
        ...
        service: "gui/510/\(UpdaterLaunchAgentInstaller.label)",
        pause: { pauses += 1 },
        revalidate: { snapshot },
        processGroupID: { $0 },
        processGroupExists: { processGroup in
            checkedGroups.append(processGroup)
            groupChecks += 1
            return groupChecks <= 2
        },
        terminateProcessGroup: { processGroup in terminateCalls.append(processGroup); return true },
        runLaunchctl: { arguments in
            commands.append(arguments)
    ...
    try expect(checkedGroups.allSatisfy { $0 == 700 }, "takeover must monitor only the predecessor's process group")
    try expectEqual(commands.map(\.first), ["bootout", "print"], "takeover must remove the whole launchd job and prove the service is absent")
    try expectEqual(pauses, 2, "main activation must wait until every process in the legacy updater group has exited")
    try expect(terminateCalls.isEmpty, "normal bootout drainage must not need an explicit group kill")

    var timedOutBootoutChecks = 0
    terminateCalls.removeAll()
    commands.removeAll()
    try LegacyUpdaterProcessGroupTakeover.run(
        snapshot: snapshot,
        expectedExecutable: executable,
        ...
        revalidate: { snapshot },
        processGroupID: { $0 },
        processGroupExists: { _ in
            timedOutBootoutChecks += 1
            return timedOutBootoutChecks == 1
        },
        terminateProcessGroup: { processGroup in terminateCalls.append(processGroup); return true },
        runLaunchctl: { arguments in
            commands.append(arguments)
    ...
    try expectEqual(terminateCalls, [700], "a timed-out bootout must explicitly drain its still-live verified process group")
    ...
    do {
        try LegacyUpdaterProcessGroupTakeover.run(
            ...
            terminateProcessGroup: { _ in true },
            runLaunchctl: { arguments in
                commands.append(arguments)
                return arguments.first == "print"
                    ? MainAppLaunchctlResult(status: 0, output: "state = not running")
                    : MainAppLaunchctlResult(status: -2, output: "process timed out")
            }
        )
        throw TestFailure(description: "a gone process with a still-loaded service must not complete takeover")
    } catch LaunchAgentUpdateBridgeError.legacyUpdaterServiceStillRegistered {
        try expectEqual(commands.map(\.first), ["bootout", "print", "bootout"], "indeterminate bootout must retry and require explicit service absence")
    }
    ...
    do {
        ...
        throw TestFailure(description: "a process group still alive after explicit kill must stop automatic repair")
    } catch LaunchAgentUpdateBridgeError.legacyUpdaterProcessGroupStillRunning(let processGroup) {
        try expectEqual(processGroup, 700, "terminal partial takeover must identify the unresolved process group")
        try expectEqual(commands.count, 1, "timed-out bootout must not be retried as proof of process-group removal")
    }

    var unsafePlist = validPlist
    unsafePlist["AbandonProcessGroup"] = true
    try PropertyListSerialization.data(fromPropertyList: unsafePlist, format: .xml, options: 0).write(to: plist)
    commands.removeAll()
    do {
        try LegacyUpdaterProcessGroupTakeover.run(
            snapshot: snapshot,
        ...
        throw TestFailure(description: "a job that abandons descendants must not be auto-bridged")
    } catch LaunchAgentUpdateBridgeError.legacyUpdaterTakeoverFailed {
        try expect(commands.isEmpty, "unsafe process-group policy must be rejected before bootout")
    }
}

func testLegacyIdleUpdaterServiceIsRemovedBeforeRecovery() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("legacy-idle-updater-removal-\(UUID().uuidString)")
    let plist = root.appendingPathComponent("com.codex.traffic-light-mxp-updater.plist")
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let validPlist: [String: Any] = ["Label": UpdaterLaunchAgentInstaller.label]
    try PropertyListSerialization.data(fromPropertyList: validPlist, format: .xml, options: 0).write(to: plist)
    let service = "gui/512/\(UpdaterLaunchAgentInstaller.label)"
    var commands: [[String]] = []
    var removed = false
    ...
    try expectEqual(commands.map(\.first), ["print", "bootout", "print"], "late recovery must remove an idle old StartInterval service and prove it is absent")
    ...
    do {
        ...
        throw TestFailure(description: "a running service without an authoritative vnode must not be removed blindly")
    } catch LaunchAgentUpdateBridgeError.legacyUpdaterServiceStillRegistered {
        try expectEqual(commands.map(\.first), ["print"], "unidentified running updater must be rejected before bootout")
    }
}

func testLateLegacyMonitorRepairsOnlyIdleTargetUpdater() throws {
    func makeFixture(_ suffix: String) throws -> (root: URL, home: URL, appRoot: URL, current: URL) {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("late-legacy-updater-repair-\(suffix)-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
        let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
        let launchAgents = home.appendingPathComponent("Library/LaunchAgents")
        ...
        try "new main".write(to: target.appendingPathComponent("CodexTrafficLightApp"), atomically: true, encoding: .utf8)
        try "new updater".write(to: target.appendingPathComponent("wanhe-status-updater"), atomically: true, encoding: .utf8)
        try "__UPDATER_PATH__".write(
            to: target.appendingPathComponent("com.codex.traffic-light-mxp-updater.plist.template"),
            atomically: true,
            encoding: .utf8
        )
        let installedUpdater: [String: Any] = [
            "Label": UpdaterLaunchAgentInstaller.label,
            "ProgramArguments": [appRoot.appendingPathComponent("current/wanhe-status-updater").path],
        ]
        try PropertyListSerialization.data(fromPropertyList: installedUpdater, format: .xml, options: 0)
            .write(to: launchAgents.appendingPathComponent("\(UpdaterLaunchAgentInstaller.label).plist"))
        let current = appRoot.appendingPathComponent("current")
        try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/\(ClientVersion.current)")
        return (root, home, appRoot, current)
    }

    do {
        let fixture = try makeFixture("idle-success")
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        var commands: [[String]] = []
        var oldRegistrationRemoved = false
        var newUpdaterBootstrapped = false
        var newUpdaterPolls = 0
        let repaired = try LaunchAgentUpdateBridge.repairCurrentUpdaterIfIdle(
            ...
            runLaunchctl: { arguments in
                commands.append(arguments)
                let mainService = "gui/513/\(MainAppLaunchAgentInstaller.label)"
                let updaterService = "gui/513/\(UpdaterLaunchAgentInstaller.label)"
                if arguments == ["print", mainService] {
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                if arguments == ["print", updaterService] {
                    if newUpdaterBootstrapped {
                        newUpdaterPolls += 1
                        return MainAppLaunchctlResult(
                            status: 0,
                            output: newUpdaterPolls == 1
                                ? "state = running"
                                : "state = exited\nlast exit code = 0"
                        )
                    }
                    return oldRegistrationRemoved
                        ? MainAppLaunchctlResult(status: 3, output: "Could not find service")
                        : MainAppLaunchctlResult(status: 0, output: "state = not running")
                }
                if arguments == ["bootout", updaterService] {
                    oldRegistrationRemoved = true
                    return MainAppLaunchctlResult(status: 0)
                }
                if arguments.first == "bootstrap",
                   arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                    newUpdaterBootstrapped = true
                    return MainAppLaunchctlResult(status: 0)
                }
                return MainAppLaunchctlResult(status: 0)
            }
        )
        try expect(repaired, "a late monitor must refresh an idle updater registration after the old updater exits")
        try expect(newUpdaterPolls >= 2, "running is not proof; repair must wait for the new one-shot updater to exit zero")
        try expectEqual(
            try FileManager.default.destinationOfSymbolicLink(atPath: fixture.current.path),
            "releases/\(ClientVersion.current)",
            "updater-only repair must never change current"
        )
        try expect(
            FileManager.default.fileExists(atPath: fixture.appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path),
            "a verified updater first run should create the target completion marker"
        )
        try expect(commands.contains(["bootout", "gui/513/\(UpdaterLaunchAgentInstaller.label)"]), "idle legacy updater registration must be removed")
    }

    do {
        let fixture = try makeFixture("running-defer")
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        var commands: [[String]] = []
        let repaired = try LaunchAgentUpdateBridge.repairCurrentUpdaterIfIdle(
            targetVersion: "\(ClientVersion.current)",
            ...
            runLaunchctl: { arguments in
                commands.append(arguments)
                if arguments == ["print", "gui/514/\(MainAppLaunchAgentInstaller.label)"] {
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                if arguments == ["print", "gui/514/\(UpdaterLaunchAgentInstaller.label)"] {
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                return MainAppLaunchctlResult(status: 0)
            }
        )
        try expect(!repaired, "a running updater must be allowed to finish or create its own bridge request")
        try expectEqual(commands.map(\.first), ["print", "print"], "running updater deferral must not bootout or bootstrap either job")
        try expect(!FileManager.default.fileExists(atPath: fixture.appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "deferral must not claim completion")
    }

    do {
        let fixture = try makeFixture("exit-failure")
        defer { try? FileManager.default.removeItem(at: fixture.root) }
        var bootstrapped = false
        var updaterPolls = 0
        do {
            _ = try LaunchAgentUpdateBridge.repairCurrentUpdaterIfIdle(
                targetVersion: "\(ClientVersion.current)",
                home: fixture.home,
                userID: 515,
                pause: {},
                runLaunchctl: { arguments in
                    let mainService = "gui/515/\(MainAppLaunchAgentInstaller.label)"
                    let updaterService = "gui/515/\(UpdaterLaunchAgentInstaller.label)"
                    if arguments == ["print", mainService] {
                        return MainAppLaunchctlResult(status: 0, output: "state = running")
                    }
                    if arguments == ["print", updaterService] {
            ...
            throw TestFailure(description: "a nonzero first updater run must fail the late repair")
        } catch LaunchAgentUpdateBridgeError.updaterDidNotRunCleanly {
            // Expected. The CLI records this target in UpdateLedger and reports
            // launch_agent_bridge_prepare_failed before returning to the monitor.
        }
        try expectEqual(
            try FileManager.default.destinationOfSymbolicLink(atPath: fixture.current.path),
            "releases/\(ClientVersion.current)",
            "failed updater-only repair must not guess or roll back a predecessor"
        )
        try expect(!FileManager.default.fileExists(atPath: fixture.appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "nonzero updater exit must remain incomplete")
    }
}

func testLaunchAgentBridgeRequiresCleanFirstUpdaterRun() throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent("launch-agent-bridge-updater-first-run-\(UUID().uuidString)")
    let home = root.appendingPathComponent("home")
    let appRoot = home.appendingPathComponent(".wanhe-codex-token/app")
    let previous = appRoot.appendingPathComponent("releases/1.2.91")
    let target = appRoot.appendingPathComponent("releases/\(ClientVersion.current)")
    ...
    do {
        _ = try LaunchAgentUpdateBridge.run(
            ...
            runLaunchctl: { arguments in
                ...
                if arguments.first == "bootstrap" {
                    if arguments.last?.hasSuffix("\(MainAppLaunchAgentInstaller.label).plist") == true {
                        absentServices.remove("gui/509/\(MainAppLaunchAgentInstaller.label)")
                    }
                    if arguments.last?.hasSuffix("\(UpdaterLaunchAgentInstaller.label).plist") == true {
                        absentServices.remove("gui/509/\(UpdaterLaunchAgentInstaller.label)")
                    }
                }
                if arguments == ["print", "gui/509/com.codex.traffic-light-mxp"] {
                    return MainAppLaunchctlResult(status: 0, output: "state = running")
                }
                if arguments.first == "bootstrap", arguments.last?.hasSuffix("com.codex.traffic-light-mxp-updater.plist") == true {
                    newUpdaterWasBootstrapped = true
                ...
                if arguments == ["print", "gui/509/com.codex.traffic-light-mxp-updater"] {
                    if newUpdaterWasBootstrapped {
                        newUpdaterPolls += 1
                    }
                    return MainAppLaunchctlResult(
                        status: 0,
                        output: !newUpdaterWasBootstrapped
                            ? "state = exited\nlast exit code = 0"
                            : (newUpdaterPolls == 1 ? "state = running" : "state = exited\nlast exit code = 78")
                    )
                }
                return MainAppLaunchctlResult(status: 0)
            },
        ...
        throw TestFailure(description: "bootstrap success alone must not authorize a done marker")
    } catch let error as LaunchAgentUpdateBridgeError {
        try expect(error.description.contains("first RunAtLoad"), "failed first updater run must remain distinguishable from bootstrap failure")
    }
    try expectEqual(try FileManager.default.destinationOfSymbolicLink(atPath: current.path), "releases/1.2.91", "unclean updater first run must roll back the whole release")
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge-\(ClientVersion.current).done").path), "unclean updater first run must not write done")
}
...
func testLegacyBridgeRequestPreSwapUsesActualCurrentRelease() throws {
    ...
    _ = try makeRelease("\(ClientVersion.current)")
...
func testLegacyBridgeRequestPostSwapUsesRunningUpdaterVnodeOnly() throws {
    ...
    let alreadyExited = try LegacyLaunchAgentBridgeRequest.prepare(
        targetVersion: "\(ClientVersion.current)",
        home: home,
        runningUpdaterExecutable: { nil }
    )
    try expectEqual(alreadyExited, nil, "post-swap preparation must not guess a predecessor after its updater has exited")
    try expect(!FileManager.default.fileExists(atPath: appRoot.appendingPathComponent("launch-agent-bridge.request").path), "post-exit discovery must not create a guessed request")
}

func testLegacyBridgeRequestIncludesSelfBridgingUpdater() throws {
    ...
    let selfBridging = try makeRelease("1.2.93")
    _ = try makeRelease("\(ClientVersion.current)")
    let current = appRoot.appendingPathComponent("current")
    try FileManager.default.createSymbolicLink(atPath: current.path, withDestinationPath: "releases/1.2.93")
    var inspectedProcess = false
    ...
    try expectEqual(preSwap, "releases/1.2.93", "pre-swap preparation must bind the request to the actual 1.2.93 current release")
    try expect(!inspectedProcess, "pre-swap preparation should trust the validated current symlink without process discovery")
    try expectEqual(
        try String(contentsOf: appRoot.appendingPathComponent("launch-agent-bridge.request"), encoding: .utf8),
        "\(ClientVersion.current)\nreleases/1.2.93\n",
        "pre-swap self-bridging predecessor must be handed to the target transaction"
    )
    ...
    try expectEqual(postSwap, "releases/1.2.93", "post-swap request must remain bound to the still-running predecessor vnode")
    try expectEqual(
        try String(contentsOf: appRoot.appendingPathComponent("launch-agent-bridge.request"), encoding: .utf8),
        "\(ClientVersion.current)\nreleases/1.2.93\n",
        "predecessor and target should converge on the same authoritative request"
    )
}
...
let tests: [(String, () throws -> Void)] = [
    ...
    ("launch agent installers retry EIO safely", testLaunchAgentInstallersRetryEIOOnlyAfterConfirmedAbsence),
    ("launch agent bridge waits and refreshes both jobs", testLaunchAgentBridgeWaitsForOldUpdaterAndRefreshesBothJobs),
    ("launch agent bridge recovers missed legacy swap", testLaunchAgentBridgeRecoversBeforeSwapAndAfterLegacyRollback),
    ("launch agent bridge rolls back safely", testLaunchAgentBridgeRollsBackAndRejectsLateUnsafeCapture),
    ("launch agent bridge refuses missing rollback", testLaunchAgentBridgeRefusesMissingRollbackRelease),
    ("launch agent bridge requires older stable previous", testLaunchAgentBridgeRejectsNonOlderPreviousBeforeLaunchd),
    ("launch agent bridge preempts self-bridging updater", testLaunchAgentBridgePreemptsSelfBridgingUpdater),
    ("launch agent bridge enforces wall-clock deadline", testLaunchAgentBridgeEnforcesWallClockDeadline),
    ("launch agent bridge stops on failed takeover", testLaunchAgentBridgeStopsWhenPredecessorTakeoverFails),
    ("legacy updater takeover drains process group", testLegacyUpdaterTakeoverWaitsForWholeProcessGroup),
    ("legacy idle updater service is removed", testLegacyIdleUpdaterServiceIsRemovedBeforeRecovery),
    ("late legacy monitor repairs idle target updater", testLateLegacyMonitorRepairsOnlyIdleTargetUpdater),
    ("launch agent bridge checks updater first run", testLaunchAgentBridgeRequiresCleanFirstUpdaterRun),
    ...
    ("legacy bridge includes self-bridging updater", testLegacyBridgeRequestIncludesSelfBridgingUpdater),
