...
public final class ProjectActivityStore {
    ...
    public func prepareSync(days: Int = 30, now: Date = Date(), codexHome: URL? = nil) -> ProjectActivitySyncReport {
        ...
        var ledger = read()
        var conversationCursors = ledger.inputEventCollectionVersion == 1 ? (ledger.conversationCursors ?? [:]) : [:]
        ...
        var ledgerChanged = previousConversationCursors != conversationCursors || ledger.inputEventCollectionVersion != 1
        ledger.inputEventCollectionVersion = 1
